...
int get_pos(char *file, int match_hour, int v, int flag, struct ret_pos_t *retpos)
{
    ...
    while (fgets(sline, sizeof(sline), fp) != NULL)
    {
        ...
        else if (match_hour > linetime.hour)
        { // 在下面
            is_b = 1;
            if (v > 0)
            {
    ...
    if (step > 0)
    {
        ...
        while (fgets(sline, sizeof(sline), fp) != NULL)
        {
            ...
            if (match_hour > linetime.hour)
            {
                ...
                if (v > 0)
                {
                    printf("  SU: [STEP] MatchHour[%d] LineHour[%d]: cpos:%ld\n", match_hour, linetime.hour, cpos);
                }
                if (cpos > epos)
                {
                    printf("  SU: [STEP] E cpos(%ld) > epos(%ld), exit.\n", cpos, epos);
                    break;
                }
            ...
            else
            {
                if (flag == 0)
                {
                    if (v > 0)
                    {
                        printf("  SE: [STEP] E MatchHour[%d] LineHour[%d] cpos:%ld\n", match_hour, linetime.hour, cpos);
                    }
                    cpos = cpos - step;
                    copy_linetime(&linetime, &linetime_last);
                    break;
                ...
                else
                {
                    if (match_hour <= linetime.hour)
                    {
                        if (v > 0)
                        {
                            printf("  SD: [STEP] E MatchHour[%d] LineHour[%d] cpos:%ld\n", match_hour, linetime.hour, cpos);
                        }
                        break;
                    }
                    if (v > 0)
                    {
                        printf("  SD: [STEP] MatchHour[%d] LineHour[%d] cpos:%ld\n", match_hour, linetime.hour, cpos);
                    }
                    cpos = cpos + step;
                    fseek(fp, cpos, SEEK_SET);
                    fgets(sline, sizeof(sline), fp);
